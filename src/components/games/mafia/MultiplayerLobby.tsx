...
export function MultiplayerLobby({
  ...
  const canStart = isHost && connectedPlayers.length >= 5
          ...
          <CardContent>
            <div className="grid grid-cols-3 gap-3 text-sm">
              <div className="flex items-center gap-2">
                ...
                <span>{settings.discussionTime}s discuss</span>
              </div>
              <div className="flex items-center gap-2">
                <Clock className="w-4 h-4 text-muted-foreground" />
                <span>{settings.votingTime}s vote</span>
              </div>
              <div className="flex items-center gap-2">
                <Moon className="w-4 h-4 text-muted-foreground" />
                <span>{settings.nightTime}s night</span>
              </div>
            </div>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            <div className="space-y-2">
              ...
              {Array.from({ length: Math.max(0, 5 - connectedPlayers.length) }).map((_, i) => (
                <div
                  key={`empty-${i}`}
        ...
        <div className="space-y-2">
          {isHost ? (
            ...
            >
              ...
              {canStart ? "Start Game" : `Need ${5 - connectedPlayers.length} more player(s)`}
            </Button>
          ) : (
