...
export function useMultiplayerMemoryMatch() {
  ...
  const connect = useCallback(
    (roomCode: string, hosting: boolean, playerName: string) => {
      ...
      const playerId = getPersistentPlayerId("memory-match", normalizedRoomCode)
      ...
      socket.addEventListener("message", (event) => {
        ...
        try {
          ...
          console.error("Failed to parse Memory Match message:", error)
        }
      })
  ...
  const disconnect = useCallback(() => {
    ...
    if (roomCodeRef.current) {
      clearPersistentPlayerId("memory-match", roomCodeRef.current)
      clearPersistentPlayerToken("memory-match", roomCodeRef.current)
      roomCodeRef.current = ""
    }
  ...
  return {
    ...
    flipCard: (index: number, roundId: string) =>
      sendNow({ type: "flip", index, roundId }),
